...
LUA_SHA1 = get_hash(os.path.join(CWD, 'codeplex.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'p':
                wget_args.extend(['--warc-header', 'codeplex-project: '+item_value])
                wget_args.append('https://archive.codeplex.com/?p='+item_value)
            else:
...
pipeline = Pipeline(
    ...
    GetItemFromTracker('http://{}/{}/multi={}/'
        ...
        downloader, VERSION),
    PrepareDirectories(warc_prefix='codeplex'),
    WgetDownload(
        WgetArgs(),
        max_tries=1,
